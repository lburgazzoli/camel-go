...
type IntegrationLister interface {
	...
	List(selector labels.Selector) (ret []*v2alpha1.Integration, err error)
...
// List lists all Integrations in the indexer.
func (s *integrationLister) List(selector labels.Selector) (ret []*v2alpha1.Integration, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2alpha1.Integration))
	})
	return ret, err
}
...
type IntegrationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2alpha1.Integration, error)
...
// List lists all Integrations in the indexer for a given namespace.
func (s integrationNamespaceLister) List(selector labels.Selector) (ret []*v2alpha1.Integration, err error) {
...
// Get retrieves the Integration from the indexer for a given namespace and name.
func (s integrationNamespaceLister) Get(name string) (*v2alpha1.Integration, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2alpha1.Resource("integration"), name)
	}
	return obj.(*v2alpha1.Integration), nil
}
