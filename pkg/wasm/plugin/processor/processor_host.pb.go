...
const ProcessorsPluginAPIVersion = 1
...
func NewProcessorsPlugin(ctx context.Context, opts ...wazeroConfigOption) (*ProcessorsPlugin, error) {
	o := &WazeroConfig{
...
func (p *ProcessorsPlugin) Load(ctx context.Context, pluginPath string) (processors, error) {
	...
	apiVersion := module.ExportedFunction("processors_api_version")
	if apiVersion == nil {
		return nil, errors.New("processors_api_version is not exported")
	}
	results, err := apiVersion.Call(ctx)
	if err != nil {
		return nil, err
	} else if len(results) != 1 {
		return nil, errors.New("invalid processors_api_version signature")
	}
	if results[0] != ProcessorsPluginAPIVersion {
		return nil, fmt.Errorf("API version mismatch, host: %d, plugin: %d", ProcessorsPluginAPIVersion, results[0])
	}

	process := module.ExportedFunction("processors_process")
	if process == nil {
		return nil, errors.New("processors_process is not exported")
	}
...
func (p *processorsPlugin) Close(ctx context.Context) (err error) {
...
func (p *processorsPlugin) Process(ctx context.Context, request *Message) (*Message, error) {
