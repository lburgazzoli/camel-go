...
func (m *Message) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ContentSchema) > 0 {
		i -= len(m.ContentSchema)
		copy(dAtA[i:], m.ContentSchema)
		i = encodeVarint(dAtA, i, uint64(len(m.ContentSchema)))
		i--
		dAtA[i] = 0x32
	}
...
func (m *Message) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			...
			m.ContentSchema = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
